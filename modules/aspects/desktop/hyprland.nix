...
}:
{
  ...
  blazar.hyprland.homeManager =
    ...
    let
      ...
      padString = n: if n < 10 then "0" + builtins.toString n else builtins.toString n;
    ...
    in
    {
      ...
      #   };
      # };
      home.file.".config/hypr/config.lua".source = ./_files/hyprland.lua;
      wayland.windowManager.hyprland.extraConfig =
        let
          ...
          namedPkgs = {
            yt = yt;
            keybindings = pkgs.writeShellScript "show-keybindings" ''
              ${self.packages.${pkgs.stdenv.hostPlatform.system}.show-keybindings}/bin/blazar-show-keybindings
            '';
            snapshot = pkgs.writeShellScript "snapshot" ''
              ${self.packages.${pkgs.stdenv.hostPlatform.system}.snapshot}/bin/blazar-snapshot
            '';
            awww = awww-script;
      ...
      #     show-keybindings = "${
      #       self.packages.${pkgs.stdenv.hostPlatform.system}.show-keybindings
      #     }/bin/blazar-show-keybindings";
      #   in
      #     [
